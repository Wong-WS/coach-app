...
const row = (
  ...
  balanceAfter: number,
  cursor: number,
): PortalChargeRow => ({ date, studentName, amount, balanceAfter, cursor });

describe('groupCharges', () => {
  it('rolls two students on the same date into one row', () => {
    const groups = groupCharges([
      row('2026-08-23', 'Jian', 50, -850, 300),
      row('2026-08-23', 'Seoan', 50, -800, 200),
      row('2026-08-21', 'Seoan', 50, -750, 100),
    ]);
  ...
  it('merges a date split across pages when the next page is appended', () => {
    const page1 = [row('2026-08-23', 'Jian', 50, -850, 300)];
    const page2 = [row('2026-08-23', 'Seoan', 50, -800, 200)];
    const groups = groupCharges([...page1, ...page2]);
    expect(groups).toHaveLength(1);
    expect(groups[0].amount).toBe(100);
    expect(groups[0].balanceAfter).toBe(-850);
  });

  it('keeps a backdated charge in its own date row', () => {
    const groups = groupCharges([
      row('2026-08-23', 'Jian', 50, -850, 300),
      row('2026-08-21', 'Seoan', 50, -900, 250),
      row('2026-08-23', 'Seoan', 50, -800, 200),
    ]);
  ...
  it('does not repeat a name when the same student has two lessons that day', () => {
    const groups = groupCharges([
      row('2026-08-23', 'Jian', 50, -100, 300),
      row('2026-08-23', 'Jian', 50, -50, 200),
    ]);
  ...
  it('leaves names empty for single-student wallets', () => {
    const groups = groupCharges([row('2026-08-23', '', 50, -100, 300)]);
